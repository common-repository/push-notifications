<?php
    ...
    * Version: 2.3.23
    *
    * Copyright 2014 Pushwoosh (email: help@pushwoosh.com)
    ...
    */
	...
	function pushwoosh_add_meta_box() {
		...
		foreach ( $post_types as $post_type ) {
			...
			add_action('future_to_publish', 'pushwoosh_publish_post');

			add_action('draft_'. $post_type, 'pushwoosh_save_post');
			add_action('pending_'. $post_type, 'pushwoosh_save_post');
			add_action('save_'. $post_type, 'pushwoosh_save_post');
		}
	}

	add_action('admin_init', 'pushwoosh_add_meta_box');
	// do not use http://codex.wordpress.org/Plugin_API/Action_Reference/save_post for publish callback
	...
	function pushwoosh_message_box($post) {
		...
		$safari_title_placeholder = 'Title';
		$checkbox_checked = '';
		$message_content = '';
		$pushwoosh_filter = '';
		$pushwoosh_filter_placeholder = 'Filter name';
		$safari_title = '';
		...
		if ($action == 'edit') {
			...
			$pushwoosh_filter = get_post_meta($post->ID, 'pushwoosh_filter', true);
		}
	...
	function pushwoosh_send_push_by_post($post_id, $message_content, $options=array()) {
		...
		$options['firefox_title'] = $options['safari_title'];
		$chrome_icon = $chrome_icon['text_string'];
		if ($chrome_icon) {
			$options['chrome_icon'] = $chrome_icon;
			$options['firefox_icon'] = $chrome_icon;
		}

		$pushwoosh  = new Pushwoosh(array('auth' => $api_token['text_string']));

		try {
			$pushwoosh->createMessage($application_code['text_string'], $message_content, $options);
			$status = 'Success';
		} catch (Exception $e) {
	...
	function pushwoosh_save_post($ID) {
		...
		if (!empty($_POST)) {
			...
			$options['pushwoosh_send_push'] = $_POST['pushwoosh_send_push'];
			$options['chrome_title'] = $options['safari_title'];
			$options['filter'] = $_POST['pushwoosh_filter'];
			// update post meta data
			update_post_meta($ID, 'pushwoosh_message_content', $message_content);
			update_post_meta($ID, 'pushwoosh_send_push', $options['pushwoosh_send_push']);
			update_post_meta($ID, 'safari_title', $options['safari_title']);
			update_post_meta($ID, 'pushwoosh_filter', $options['filter']);
		}
 	...
 	function pushwoosh_publish_post($post) {
		...
		if (!empty($_POST)) {
			$postOptions = pushwoosh_save_post($post_id);
			if (false === $postOptions) {
				return;
			}
			...
			$options['filter'] = get_post_meta($post_id, 'pushwoosh_filter', true);
		}

		if (!get_post_meta($post_id, 'pushwoosh_send_push', true)) {
			return;
		}
